...
If the window is too big or the letters too small
=================================================

You can edit `main.py` to change the font size. Just change the variable
`FONT_SIZE` (at the very top of the file) to a different number.
...
What do those file names even mean?
===================================

Every file generated by the tool is a separate dataset. The name of the
file will look something like this:

```
fakedata_sub10_cond2_tri10_dp300_fx100_rdpnoise10_rprobnoise0.01_rdsnoise2_sptdpvar7_sptprobvar0.005_sptdsvar2_sdpintrsd15_sdpslopsd5_sprobsd0.05_sdssd4_idpsd15_ipsd0.05_idssd5_olp0.01_solpsd0.001_0ctscq.txt
```

The name of the file indicates the values that were given to the
generator in order to produce it. Here is a list of what they mean
(hopefully you can see how they are directly connected to the
values you set when you ran the program):

 * `sub10`: 10 Participants
 * `cond2`: 2 Conditions
 * `tri10`: 10 Trials
 * `dp300`: Div. Point happens at 300ms
 * `fx100`: Effect of Condition is 100ms
 * `rdpnoise10`: Random Variation in Div. Point is 10
 * `rprobnoise0.01`: Random Variation in Prob. of looks to Target is 0.01
 * `rdsnoise2`: Random Variation in "Div. Speed" is 2
 * `sptdpvar7`: Per Participant per trial Variation in Div. Point is 7
 * `sptprobvar0.005`: Per Participant per trial Variation in Prob. of looks to Target is 0.005
 * `sptdsvar2`: Per Participant per trial Variation in "Div. Speed" is 2
 * `sdpintrsd15`: Per Participant Variation in Random Intercepts is 15
 * `sdpslopsd5`: Per Participant Variation in Random Slopes is 5
 * `sprobsd0.05`: Per Participant Variation in Prob. of looks to Target is 0.05
 * `sdssd4`: Per Participant Variation in "Div. Speed" is 4
 * `idpsd15`: Per Item Variation in Div. Point is 15
 * `ipsd0.05`: Per Item Variation in Prob. of looks to Target is 0.05
 * `idssd5`: Per Item Variation in "Div. Speed" is 5
 * `olp0.01`: Overall Probability of looking away is 0.01
 * `solpsd0.001`: Per Participant Variation of looking away is 0.001
 * `0ctscq`: This is the random seed used to generate the file
 * `.txt`: the extension of the produced file
